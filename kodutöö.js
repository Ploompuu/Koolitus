...
function valjastaArvamisteAjalugu() {
    ...
    for (index = 0; index < arvatudVarvid.length; index++) {
        valjundEl.innerHTML += `
        <div>${index}. arvamine - ${arvatudVarvid[index]}</div>`
...
function varviarvamine() {
    ...
    if (lemmikVarvid.includes(sisendTekst)) {
        valjastaTeade("Õnnitlused - arvasid äraminu lemmik värvi" + sisendTekst);
